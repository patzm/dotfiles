...
alias skhdconfig='nvim ${XDG_CONFIG_HOME}/skhd/skhdrc'
alias sshconfig='nvim ${HOME}/.ssh/config'
alias tmuxconfig='nvim ${HOME}/.tmux.conf'
alias vimconfig='nvim ${HOME}/.vim/vimrc'
alias yabaiconfig='nvim ${XDG_CONFIG_HOME}/yabai/yabairc'
alias yabai-post-upgrade='echo "$(whoami) ALL=(root) NOPASSWD: sha256:$(shasum -a 256 $(which yabai) | cut -d " " -f 1) $(which yabai) --load-sa" | sudo tee /private/etc/sudoers.d/yabai > /dev/null && yabai --restart-service'
